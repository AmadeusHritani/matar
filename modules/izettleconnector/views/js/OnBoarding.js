...
class IzettleOnBoarding
{
   ...
   * @param {boolean} isShutDown   Did the IzettleOnBoarding is shut down ?
   * @param {string}  apiLocation  IzettleOnBoarding API location
  ...
  showCurrentStep()
  {
    ...
    if (!this.isShutDown) {
      ...
      if (null != step.action && step.action.before_show) {
        setTimeout(function (){$(step.action.selector)[step.action.action]();}, 0);
      }

      if (IzettleOnBoarding.isCurrentPage(step.page)) {
  ...
  gotoStep(stepIndex)
  {
    this.save({action: 'setCurrentStep', value: stepIndex}, ((error) => {
      if (!error) {
        ...
        if (null == currentStep.action || (null != currentStep.keep_going_after_action && currentStep.keep_going_after_action)){
          this.currentStep++;
          if (!IzettleOnBoarding.isCurrentPage(nextStep.page)) {
  ...
  getTokenAsString(redirectUrl) {
    ...
    var tokens = IzettleOnBoarding.getSecurityTokens(queryString, redirectUrl);
  ...
  static getSecurityTokens(queryString, redirectUrl) {
    var queryParams = IzettleOnBoarding.parseQueryString(queryString);
    var tokens = {};
    ...
    this.save({action: 'setShutDown', value: this.isShutDown}, ((error) => {
      if (!error) {
        if (0 === this.isShutDown) {
          if (IzettleOnBoarding.isCurrentPage(this.getStep(this.currentStep).page)) {
            this.showCurrentStep();
          } else {
   ...
   */
   ...
   {
     var middleX = this.tooltipElement.offset().left + (this.tooltipElement.outerWidth() / 2) - (this.tooltip.outerWidth() / 2);
     var middleY = this.tooltipElement.offset().top + (this.tooltipElement.outerHeight() / 2) - (this.tooltip.outerHeight() / 2);
     var topY = this.tooltipElement.offset().top  - (this.tooltip.outerHeight() );
