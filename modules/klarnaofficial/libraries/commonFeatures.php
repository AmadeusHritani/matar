...
class KlarnaCheckoutCommonFeatures
{
    public function BuildCartArray($cart, $shippingReference, $wrappingreference, $wrappingname, $discountname) {
        ...
        if ($shipping_cost_without_tax > 0) {
            ...
            $carriername = strip_tags($carrier->name);
            if ("" == $carriername) {
                $carriername = $shippingReference;
            }
    ...
    public function BuildShippingOptions($cart)
    {
        $carrieraddress = new Address($cart->id_address_delivery);
        ...
        foreach ($cart->getDeliveryOptionList(null, true) as $options) {
            foreach ($options as $option) {
                foreach ($option["carrier_list"] as $carrieroption) {
                    ...
                    // calculate tax amount with percentage and with tax price. Prestashop does not give the correct
                    // price_without_tax
                    $taxamount = $option["total_price_with_tax"] - ($option["total_price_with_tax"] / (1 + ($shipping_tax_rate/100)));
                    $shipping_option["tax_amount"] = Tools::ps_round($taxamount * 100, 0);
