...
if (!defined('_PS_VERSION_'))
	exit;
class Ph_pcms_obj extends ObjectModel 
{
    public function renderForm()
    {
        $this->fields = $this->getListFields();
        $helper = new HelperForm();
        $helper->module = new Ph_products_cms();
        ...
        if($configs)
        {
            ...
            {                
                ...
                $confFields = array(
                    ...
                    'display_img' => $this->id && isset($config['type']) && $config['type']=='file' && $this->$key!='' && @file_exists(_PS_PH_PCMS_IMG_DIR_.$this->$key) ? _PS_ets_pr_IMG_.$this->$key : false,
                    'img_del_link' => $this->id && isset($config['type']) && $config['type']=='file' && $this->$key!='' && @file_exists(_PS_PH_PCMS_IMG_DIR_.$this->$key) ? $this->context->link->getAdminBaseLink('AdminModules').'&configure=ets_promotion&deleteimage='.$key.'&itemId='.(isset($this->id)?$this->id:'0').'&Ets_pr_object=PRMN_'.Tools::ucfirst($fields_form['form']['name']) : false,
    ...
    public function saveData()
    {
        ...
        if($configs)
        {
            foreach($configs as $key => $config)
            {
                ...
                if(isset($config['lang']) && $config['lang'])
                {
                    ...
                    {
                        $errors[] = sprintf($this->l('%s is required','ph_pcms_obj'),$config['label']);
                    }
                    elseif($key_value_lang_default!='' && isset($config['validate']) && ($config['validate']=='isColor' || $config['validate']=='isLink'))
                    {
                        ...
                        if(!Ets_promotion::$validate(trim($key_value_lang_default)))
                            $errors[] = sprintf($this->l('%s is not valid','ph_pcms_obj'),$config['label']);
                        unset($validate);
                    }
                            ...
                            if($language['id_lang']!=$id_lang_default)
                            {
                                ...
                                if($value_lang!='' && isset($config['validate']) && ($config['validate']=='isColor' || $config['validate']=='isLink'))
                                {
                                    ...
                                    if(!Ets_promotion::$validate(trim($value_lang)))
                                        $errors[] = sprintf($this->l('%s is not valid in %s','ph_pcms_obj'),$config['label'],$language['iso_code']);
                                    unset($validate);
                                }
                ...
                elseif($config['type']=='file_lang')
                {
                    ...
                    foreach($languages as $l)
                    {
                        ...
                        if(isset($_FILES[$name]['tmp_name']) && isset($_FILES[$name]['name']) && $_FILES[$name]['name'])
                        {
                            ...
                            $imageName = @file_exists(_PS_PH_PCMS_IMG_DIR_.Tools::strtolower($_FILES[$name]['name'])) ? Tools::passwdGen().'-'.Tools::strtolower($_FILES[$name]['name']) : Tools::strtolower($_FILES[$name]['name']);
                            $fileName = _PS_PH_PCMS_IMG_DIR_.$imageName;  
                            ...
                            elseif($_FILES[$name]['size'] > $max_file_size)
                                $errors[] = sprintf($this->l('%s file is too large','ph_pcms_obj'),$config['label']);
                            elseif(file_exists($fileName))
                            {
                                $errors[] =sprintf($this->l('%s file is already existed','ph_pcms_obj'),$config['label']);
                            }
                            else
                            {                                    
                    			...
                    			)
                    			{
                    				...
                    				elseif (!$temp_name || !move_uploaded_file($_FILES[$name]['tmp_name'], $temp_name))
                    					$errors[] = sprintf($this->l('%s cannot upload in %s','ph_pcms_obj'),$config['label'],$l['iso_code']);
                    				elseif (!ImageManager::resize($temp_name, $fileName, null, null, $type))
                    					$errors[] = printf($this->l('%s An error occurred during the image upload process in %s','ph_pcms_obj'),$config['label'],$l['iso_code']);
                    				if (isset($temp_name))
                    					@unlink($temp_name);
                                    if(!$errors)
                                    {
                                    ...
                                    $errors[] = sprintf($this->l('%s file in %s is not in the correct format, accepted formats: jpg, gif, jpeg, png.','ph_pcms_obj'),$config['label']);
                            }
                        }
                    }
                }
        ...
        { 
            ...
            {
                $success[] = $this->l('Save successfully','ph_pcms_obj');
                if($old_files)
                {
                            ...
                            if(is_array($file))
                            {
                                foreach($file as $f)
                                {
                                    if(!in_array($f,$this->$key_file))
                                        @unlink(_PS_PH_PCMS_IMG_DIR_.$f);
                                }
                            }
                            else
                                @unlink(_PS_PH_PCMS_IMG_DIR_.$file);
                        }
                    }
                }
            }                
            else
            {
                ...
                $errors[] = $this->l('Saving failed','ph_pcms_obj');
            }
        }
