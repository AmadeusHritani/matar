...
<div class="dtm-right-block">
    ...
    <div id="import_history" class="tab_content import{if $tab_history=='import'} active{/if}">
                    ...
                    <tr>
                        ...
                        <td>
                            {if $pres2pres_import_last==$import.id_import_history && $import.ok_import}
                                <a href="index.php?tab=AdminModules&configure=ets_pres2pres&token={$token|escape:'html':'UTF-8'}&tab_module=front_office_features&module_name=ets_pres2pres&tabmodule=import&resumeImport&id_import_history={$import.id_import_history|intval}"><i class="fa fa-undo"> </i> {l s='Resume migration' mod='ets_pres2presfree'}</a><br />
                            {/if}
                                <a href="index.php?tab=AdminModules&configure=ets_pres2pres&token={$token|escape:'html':'UTF-8'}&tab_module=front_office_features&module_name=ets_pres2pres&tabmodule=import&restartImport&id_import_history={$import.id_import_history|intval}"><i class="fa fa-refresh"> </i> {l s='Restart migration' mod='ets_pres2presfree'}</a><br />
                                <a href="index.php?tab=AdminModules&configure=ets_pres2pres&token={$token|escape:'html':'UTF-8'}&tab_module=front_office_features&module_name=ets_pres2pres&tabmodule=history&deleteimporthistory&id_import_history={$import.id_import_history|intval}"><i class="fa fa-trash"> </i> {l s='Delete' mod='ets_pres2presfree'}</a><br />
                            {if $import.new_passwd_customer}
                                <a href="index.php?tab=AdminModules&configure=ets_pres2pres&token={$token|escape:'html':'UTF-8'}&tab_module=front_office_features&module_name=ets_pres2pres&tabmodule=history&downloadpasscustomer&id_import_history={$import.id_import_history|intval}"><i class="fa fa-cloud-download"> </i> {l s='Download new password customer' mod='ets_pres2presfree'}</a><br />
                            {/if}
                            {if $import.new_passwd_employee}
                                <a href="index.php?tab=AdminModules&configure=ets_pres2pres&token={$token|escape:'html':'UTF-8'}&tab_module=front_office_features&module_name=ets_pres2pres&tabmodule=history&downloadpassemployee&id_import_history={$import.id_import_history|intval}"><i class="fa fa-cloud-download"> </i> {l s='Download new password employee' mod='ets_pres2presfree'}</a><br />
                            {/if}
                        </td>
                    </tr>
                {/foreach}
        ...
        {else}
            <div class="no-have-history">{l s='Migration history is empty' mod='ets_pres2presfree'}</div>
        {/if}
    </div>
</div>
