...
class Pres2PresExtraImport extends Module
{
    public $_module;
    public $imported;
    ...
    {
        $this->imported= (int)Db::getInstance()->getValue('SELECT number_import FROM '._DB_PREFIX_.'ets_pres2pres_import_history WHERE id_import_history='.(int)$this->context->cookie->id_import_history);
        $readXML = new Pres2PresReadXML($file_xml);
        while($xml = $readXML->_readXML())
		{	
		    Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'product_tag');  
            if($xml->product_tag)
            {
        ...
        if($readXML->imported)
            Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'ets_pres2pres_import_history set currentindex=1 WHERE id_import_history='.(int)$this->context->cookie->id_import_history);
    }
    public function ImportCustomizedData($file_xml)
    {
        ...
        while($xml = $readXML->_readXML())
		{	
		    Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'customized_data');  
    ...
    public function importCategoryGroup($category=false)
    {
        ...
        if($category)
        {
            $categories= Db::getInstance()->executeS('SELECT * FROM '._DB_PREFIX_.'ets_pres2pres_category_import WHERE id_import_history='.(int)$this->context->cookie->id_import_history,true,0);
            if($categories)
            {
        ...
        {
            $groups= Db::getInstance()->executeS('SELECT * FROM '._DB_PREFIX_.'ets_pres2pres_group_import WHERE id_import_history='.(int)$this->context->cookie->id_import_history);
            if($groups)
            {
                foreach($groups as $group)
                {
    ...
    public function importCustomerGroup($file_xml)
    {
        ...
        while($xml = $readXML->_readXML())
		{	
            Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'customer_group');
    ...
    public function importProductCategory($file_xml)
    {
        ...
        while($xml = $readXML->_readXML())
		{
            Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'category_product');
            if($xml->category_product)
            {
                foreach($xml->category_product as $productcategory_xml)
                {
                    ...
                    if(!$id_category)
                    {
                        ...
                        $id_category = Db::getInstance()->getValue('SELECT id_category_default FROM '._DB_PREFIX_.'ets_pres2pres_import_history WHERE id_import_history ="'.(int)$id_import_history.'"');
                    }
    ...
    public function importFeatureProduct($file_xml)
    {
        ...
        while($xml = $readXML->_readXML())
		{
            Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'feature_product');
    ...
    public function importProductAttributeCombination($file_xml)
    {
        ...
        while($xml = $readXML->_readXML())
		{	
            Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'product_attribute_combination');
    ...
    public function importProductCarrier($file_xml)
    {
        ...
        while($xml = $readXML->_readXML())
		{	
		    Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'product_carrier');  
    ...
    public function importProductSupplier($file_xml)
    {
        ...
        while($xml = $readXML->_readXML())
		{	
            Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'product_supplier');
    ...
    public function ImportProductAttributeImages($file_xml)
    {
        ...
        while($xml = $readXML->_readXML())
		{
		    Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'product_attribute_image');  
    ...
    {
        $this->imported= (int)Db::getInstance()->getValue('SELECT number_import FROM '._DB_PREFIX_.'ets_pres2pres_import_history WHERE id_import_history='.(int)$this->context->cookie->id_import_history);
        $carriers= Db::getInstance()->executeS('SELECT id_new FROM '._DB_PREFIX_.'ets_pres2pres_carrier_import WHERE id_import_history='.(int)$this->context->cookie->id_import_history,true,0);
        if($carriers)
        {
    ...
    public function importProductPack($file_xml){
        $this->imported= (int)Db::getInstance()->getValue('SELECT number_import FROM '._DB_PREFIX_.'ets_pres2pres_import_history WHERE id_import_history='.(int)$this->context->cookie->id_import_history);
        $readXML = new Pres2PresReadXML($file_xml);
        while($xml = $readXML->_readXML())
		{
		    Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'pack');
            if($xml->pack)
            {
                foreach($xml->pack as $pack_xml)
                {
                    $id_product_pack = $this->_module->getNewID('product',(int)$pack_xml->id_product_pack);
                    $id_product_item = $this->_module->getNewID('product',(int)$pack_xml->id_product_item);
                    if(isset($pack_xml->id_product_attribute_item))
                        $id_product_attribute_item =$this->_module->getNewID('product_attribute',(int)$pack_xml->id_product_attribute_item);
                    else
                        $id_product_attribute_item=0;
                    if(version_compare(_PS_VERSION_, '1.6', '>='))
                    {
                        if(!Db::getInstance()->getRow('SELECT * FROM '._DB_PREFIX_.'pack WHERE id_product_pack="'.(int)$id_product_pack.'" AND id_product_item="'.(int)$id_product_item.'" AND id_product_attribute_item="'.(int)$id_product_attribute_item.'"',0))
                        {
                            Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'pack(id_product_pack,id_product_item,id_product_attribute_item,quantity) values("'.(int)$id_product_pack.'","'.(int)$id_product_item.'","'.(int)$id_product_attribute_item.'","'.(int)$pack_xml->quantity.'")');
                            $this->stopResetImport();
                        }
                    }
                    else
                    {
                        if(!Db::getInstance()->getRow('SELECT * FROM '._DB_PREFIX_.'pack WHERE id_product_pack="'.(int)$id_product_pack.'" AND id_product_item="'.(int)$id_product_item.'"',0))
                        {
                            Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'pack(id_product_pack,id_product_item,quantity) values("'.(int)$id_product_pack.'","'.(int)$id_product_item.'","'.(int)$pack_xml->quantity.'")');
                            $this->stopResetImport();
                        }
                    }
                }
    ...
    public function importCarrierZone($file_xml)
    {
        ...
        while($xml = $readXML->_readXML())
		{
            Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'carrier_zone');
            if($xml->carrier_zone)
            {
    ...
    public function importDataQuantity14($file_xml)
    {
        ...
        while($xml = $readXML->_readXML())
		{	
            Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'product_attribute');
    ...
    {
        $this->imported++;
        Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'ets_pres2pres_import_history SET number_import ="'.(int)$this->imported.'" WHERE id_import_history='.(int)$this->context->cookie->id_import_history);
    ...
    public function processGenerate($options)
    {
        ...
        if (count($tab) && Validate::isLoadedObject($this->product)) {
            Pres2PresExtraImport::setAttributesImpacts($this->product->id, $tab);
            $this->combinations = array_values(Pres2PresExtraImport::createCombinations($tab));
    ...
    protected static function createCombinations($list)
    {
        ...
        foreach ($first as $attribute) {
            $tab = Pres2PresExtraImport::createCombinations($list);
    ...
    public function importAccessory($file_xml)
    {
        ...
        while($xml = $readXML->_readXML())
		{
            Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.'accessory');
            if($xml->accessory)
            {
