...
if(!class_exists('Pres2PresReadXML'))
    include_once(_PS_MODULE_DIR_.'ets_pres2presfree/classes/ReadXMl.php');
class Pres2PresDataImport extends Module
{
    public $_module;
    ...
    public function importData($file_xml,$class_name,$definition,$foreign_key=array(),$multishop=false)
    {
        ...
        $readXml = new Pres2PresReadXML($file_xml);
		...
		{	
            ...
            {
                $check_import = Db::getInstance()->getRow('SELECT * FROM '._DB_PREFIX_.'ets_pres2pres_'.pSQL($table).'_import where id_import_history='.(int)$id_import_history);
                if(!$check_import)
                {
                    ...
                    {
                        Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.pSQL($table).' WHERE id_product in (SELECT id_new FROM '._DB_PREFIX_.'ets_pres2pres_product_import WHERE id_import_history="'.(int)$id_import_history.'") AND '.pSQL($primary).' NOT IN (SELECT id_new FROM '._DB_PREFIX_.'ets_pres2pres_'.pSQL($table).'_import WHERE id_import_history ="'.(int)$id_import_history.'")');
                    }
                }
                foreach($xml->$table as $data_xml)
                {
                    $object_id=0;
                    $imported= (int)Db::getInstance()->getValue('SELECT number_import FROM '._DB_PREFIX_.'ets_pres2pres_import_history WHERE id_import_history='.(int)$id_import_history,0);
                    if(!(int)Db::getInstance()->getValue('SELECT id_new FROM '._DB_PREFIX_.'ets_pres2pres_'.pSQL($table).'_import WHERE id_old='.(int)$data_xml->$primary.' AND id_import_history="'.(int)$id_import_history.'"',false))
                    {
                        if($class_name=='Employee')
                        {
                            ...
                            {
                                Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'ets_pres2pres_'.pSQL($table).'_import values("","'.(int)$data_xml->$primary.'","'.(int)$id_employee.'","'.(int)$id_import_history.'")');
                                $imported++;
                                Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'ets_pres2pres_import_history SET number_import ="'.(int)$imported.'" WHERE id_import_history='.(int)$id_import_history);
                                continue;
                            }  
                        }
                        if($class_name=='Customer'&& !(int)$data_xml->is_guest && !(int)$data_xml->deleted)
                        {
                            ...
                            {
                                Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'ets_pres2pres_'.pSQL($table).'_import values("","'.(int)$data_xml->$primary.'","'.(int)$id_customer.'","'.(int)$id_import_history.'")');
                                $imported++;
                                Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'ets_pres2pres_import_history SET number_import ="'.(int)$imported.'" WHERE id_import_history='.(int)$id_import_history);
                                continue;
                            }  
                        ...
                        else
                        {
                            ...
                            if($import_history->force_all_id_number && !$object_id && $class_name!='Employee' && $class_name!='Carrier' && $class_name!='Currency' && $class_name!='Country')
                            {
                                if(isset($definition['multilang_shop'])&& $definition['multilang_shop'] && !$multishop)
                                {
                                    $object_id = (int)Db::getInstance()->getValue('SELECT tb.'.pSQL($primary).' FROM '._DB_PREFIX_.pSQL($table).' tb, '._DB_PREFIX_.pSQL($table).'_shop tbs WHERE tb.'.pSQL($primary).'='.(int)$data_xml->$primary.' AND tb.'.pSQL($primary).' = tbs.'.pSQL($primary).' AND tbs.id_shop='.(int)Context::getContext()->shop->id);
                        ...
                        if($class_name=='Group')
                        {
                            ...
                            {
                                Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'ets_pres2pres_'.pSQL($table).'_import values("","'.(int)$data_xml->$primary.'","'.(int)$object_id.'","'.(int)$id_import_history.'")');
                                $imported++;
                                Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'ets_pres2pres_import_history SET number_import ="'.(int)$imported.'" WHERE id_import_history='.(int)$id_import_history);
                                continue;
                            }
                        ...
                        if($foreign_key)
                        {
                            foreach($foreign_key as $key=>$value)
                            {
                                ...
                                {
                                    if($key_extra = (int)Db::getInstance()->getValue('SELECT id_new FROM '._DB_PREFIX_.'ets_pres2pres_'.pSQL($value['table_parent']).'_import WHERE id_old='.(int)$object->$key.' AND id_import_history="'.(int)$id_import_history.'"',0))
                                    {
                                        $object->$key = $key_extra;
                                    }
                                    elseif(!Db::getInstance()->getRow('SELECT id_new FROM '._DB_PREFIX_.'ets_pres2pres_'.pSQL($value['table_parent']).'_import WHERE id_import_history="'.(int)$id_import_history.'"'))
                                    {
                                        ...
                                        elseif($key=='id_cart' && $class_name=='Order')
                                        {
                                            $object->id_cart = (int)$data_xml->id_cart;
                                        }
                                    }
                                    elseif($key=='id_cart' && $class_name=='Order')
                                    {
                        ...
                        if($class_name=='Customer' && $object->is_guest==0)
                        {
                            if(Configuration::get('ETS_PRES2PRES_NEW_PASSWD'))
                            {
                                ...
                                if($id_ets_pres2pres_customer_pasword=Db::getInstance()->getValue('SELECT id_ets_pres2pres_customer_pasword FROM '._DB_PREFIX_.'ets_pres2pres_customer_pasword WHERE id_import_history="'.(int)$id_import_history.'" AND email="'.pSQL($object->email).'"'))
                                    Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'ets_pres2pres_customer_pasword set passwd="'.pSQL($new_passwd).'" where id_ets_pres2pres_customer_pasword="'.(int)$id_ets_pres2pres_customer_pasword.'"');
                                else
                                    Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'ets_pres2pres_customer_pasword(id_import_history,first_name,last_name,email,passwd) VALUES("'.(int)$id_import_history.'","'.pSQL($object->firstname).'","'.pSQL($object->lastname).'","'.pSQL($object->email).'","'.pSQL($new_passwd).'")');
                            }
                            else
                            {
                                $fieldsCustomers = Db::getInstance()->ExecuteS('DESCRIBE '._DB_PREFIX_.'customer');
                                $check_add=true;
                                foreach($fieldsCustomers as $field)
                                {
                                    if($field['Field']=='passwd_old_wp')
                                    {
                                        $check_add=false;
                                        break;
                                    }    
                                ...
                                if($check_add)
                                    Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'customer` ADD  `passwd_old_wp` VARCHAR(60) NOT NULL ');
                            }
                        }  
                        if($class_name=='Employee')
                        {
                            if(Configuration::get('ETS_PRES2PRES_NEW_PASSWD'))
                            {
                                $new_passwd = $this->_module->genSecure(8);
                                $object->passwd = md5(pSQL(_COOKIE_KEY_.$new_passwd));
                                if($id_ets_pres2pres_employee_pasword=Db::getInstance()->getValue('SELECT id_ets_pres2pres_employee_pasword FROM '._DB_PREFIX_.'ets_pres2pres_employee_pasword WHERE id_import_history="'.(int)$id_import_history.'" AND email="'.pSQL($object->email).'"'))
                                    Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'ets_pres2pres_employee_pasword set passwd="'.pSQL($new_passwd).'" where id_ets_pres2pres_employee_pasword="'.(int)$id_ets_pres2pres_employee_pasword.'"');
                                else
                                    Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'ets_pres2pres_employee_pasword(id_import_history,first_name,last_name,email,passwd) VALUES("'.(int)$id_import_history.'","'.pSQL($object->firstname).'","'.pSQL($object->lastname).'","'.pSQL($object->email).'","'.pSQL($new_passwd).'")');
                            }
                            else
                            {
                                $fieldsEmployees = Db::getInstance()->ExecuteS('DESCRIBE '._DB_PREFIX_.'employee');
                                $check_add=true;
                                foreach($fieldsEmployees as $field)
                                {
                                    if($field['Field']=='passwd_old_wp')
                                    {
                                        $check_add=false;
                                ...
                                if($check_add)
                                    Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'employee` ADD  `passwd_old_wp` VARCHAR(60) NOT NULL ');
                            }
                        }
                        if($class_name=='CustomizationField')
                        {
                        ...
                        if($fields)
                        {
                            ...
                            foreach($fields as $key=> $field)
                            {
                                if(!isset($field['lang']) || !$field['lang'])
                                {
                                    if(isset($field['required'])&&$field['required'] && ($object->$key=='' || ($class_name=='Order' && isset($field['validate']) && $field['validate']=='isUnsignedId' && $object->$key==0))  && $key!='id_product_attribute')
                                    {
                                        ...
                                        elseif($key=='id_cart' && $class_name=='Order')
                                            $object->$key=1;
                                        elseif($key=='email')
                                            $object->$key = 'customer'.(int)$data_xml->$primary.'@example.xyz';
                                        elseif($key=='passwd')
                                            $object->$key = md5('psa');
                                        elseif(isset($field['validate']) && ($field['validate']=='isName' || $field['validate']=='isAddress' || $field['validate']=='isCityName' || $field['validate']=='isGenericName' || $field['validate']=='isCarrierName' || $field['validate']=='isCatalogName' || $field['validate']=='isLinkRewrite'))
                                            $object->$key = $key;
                                        else
                                        {
                                             $ok=false;
                            ...
                            {
                                if(!$object->phone && !$object->phone_mobile)
                                {
                        ...
                        if(isset($definition['multilang_shop'])&& $definition['multilang_shop'] && !$multishop)
                        {
                            if((int)Db::getInstance()->getValue('SELECT '.pSQL($primary).' FROM '._DB_PREFIX_.pSQL($table).'_shop WHERE id_shop!="'.(int)Context::getContext()->shop->id.'" AND '.pSQL($primary).'="'.(int)$data_xml->$primary.'"'))
                                $object->id=0;
                        }
                        if(($class_name=='Category'|| $class_name=='CMSCategory') && $object->id==$object->id_parent)
                            unset($object->id);
                        if(isset($object->id)&&$object->id)
                            $object->update(true);
                        else
                        {
                            if($import_history->force_all_id_number && $class_name!='Employee' && $class_name!='Carrier' && $class_name!='Currency' && $class_name!='Country')
                            {
                            ...
                            if($class_name=='StockAvailable'&&!$multishop)
                            {
                        ...
                        if(($class_name=='Customer' && !Configuration::get('ETS_PRES2PRES_NEW_PASSWD'))||($class_name=='Employee' && !Configuration::get('ETS_PRES2PRES_NEW_PASSWD')))
                        {
                        ...
                        $imported++;
                        Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'ets_pres2pres_import_history SET number_import ="'.(int)$imported.'" WHERE id_import_history='.(int)$id_import_history);
                        Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'ets_pres2pres_'.pSQL($table).'_import values("","'.(int)$data_xml->$primary.'","'.(int)$object->id.'","'.(int)$id_import_history.'")');
                        ...
                        if($class_name=='Carrier' && isset($data_xml->id_reference) && (int)$data_xml->id_reference)
                        {
                            ...
                            $id_reference_new = (int)Db::getInstance()->getValue('SELECT id_new FROM '._DB_PREFIX_.'ets_pres2pres_reference_import WHERE id_old='.(int)$data_xml->id_reference.' AND id_import_history='.(int)$id_import_history,0);
                            if($id_reference_new)
                            {
                            ...
                            {
                                Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'ets_pres2pres_reference_import values("","'.(int)$data_xml->id_reference.'","'.(int)$id_reference.'","'.(int)$id_import_history.'")');
                            }
                        }
                        if(isset($definition['multilang_shop']) && $definition['multilang_shop'])
                        {
                            if(isset($data_xml->datashop) && $data_xml->datashop)
                            {
                                ...
                                if($multishop)
                                {
                                    foreach($data_xml->datashop as $datashop)
                                    {
                                        ...
                                        $id_shop_new = Db::getInstance()->getValue('SELECT id_new FROM '._DB_PREFIX_.'ets_pres2pres_shop_import WHERE id_old='.(int)$id_shop_old.' AND id_import_history="'.(int)$id_import_history.'"',0);
                                            ...
                                            foreach($fields as $key=> $field)
                                            {
                                                if(isset($field['shop']) && $field['shop'])
                                                {
                                                    if(isset($foreign_key[$key]) && $foreign_key[$key] && isset($datashop[$key])&& $datashop[$key])
                                                    {
                                                        ...
                                                        if($key_extra = (int)Db::getInstance()->getValue('SELECT id_new FROM '._DB_PREFIX_.'ets_pres2pres_'.pSQL($value['table_parent']).'_import WHERE id_old='.(int)$datashop[$key].' AND id_import_history="'.(int)$id_import_history.'"',0))
                                                        {
                        ...
                        if(isset($definition['multilang']) && $definition['multilang']  && isset($data_xml->datalanguage) && $data_xml->datalanguage)
                        {
                           ...
                           foreach($data_xml->datalanguage as $datalanguage)
                           {
                            ...
                            if(isset($list_language_xml) && $list_language_xml)
                            {
                                foreach($languages as $language)
                                {
                                    if(!in_array($language['id_lang'],$list_language_xml))
                                    {
                                        if(isset($definition['multilang_temp'])&& $definition['multilang_temp'])
                                        {
                                            if($multishop)
                                            {
                                                $id_shop_old = (int)$language_xml_default->id_shop;
                                                $id_shop_new = Db::getInstance()->getValue('SELECT id_new FROM '._DB_PREFIX_.'ets_shop_import WHERE id_old='.(int)$id_shop_old.' AND id_import_history="'.(int)$id_import_history.'"',0);
                                                $id_lang = $language['id_lang'];
                                                if(!Db::getInstance()->executeS('SELECT * FROM '._DB_PREFIX_.pSQL($table).'_lang WHERE id_shop ="'.(int)$id_shop_new.'" AND '.pSQL($primary).'="'.(int)$object->id.'" AND id_lang="'.(int)$id_lang.'"'))
                                                {
                                                    $sql_lang = 'INSERT INTO '._DB_PREFIX_.pSQL($table).'_lang SET '.pSQL($primary).'="'.(int)$object->id.'",'.(isset($definition['multilang_temp'])&& $definition['multilang_temp']?'id_shop="'.(int)$id_shop_new.'",':'').'id_lang="'.(int)$id_lang.'"';
                                                    foreach($fields as $key=> $field)
                                                    {
                                                        if(isset($field['lang']) && $field['lang']&& isset($language_xml_default->$key)&& $language_xml_default->$key)
                                                        {
                                                            $sql_lang .=',`'.pSQL($key).'`="'.pSQL((string)$language_xml_default->$key,true).'"';
                                                        }
                                                    }
                                                    Db::getInstance()->execute($sql_lang);
                                                }   
                        ...
                        if($class_name=='Category' &&  isset($data_xml->link_thumb))
                        {
                            $url = str_replace(' ', '%20', (string)$data_xml->link_thumb);
                            $this->copyImg($object->id,null,$url,$class_name,true,true);
                        }
                        if($class_name=='Carrier' && $object->id && isset($data_xml->link_image) && (string)$data_xml->link_image)
                        {
                            ...
                            Pres2PresDataImport::copy($url, _PS_SHIP_IMG_DIR_ . '/' . (int) $object->id . '.jpg',$context);
                        }
                        Configuration::updateValue('PS_PRES2PRES_IMPORTING',_DB_PREFIX_.$table);
                        if($imported%100==0)
                        {                            
                            die('imported='.$imported);
                        }
                    }
                }
            }
        ...
        if($readXml->imported)
        {
            Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'ets_pres2pres_import_history SET currentindex =1 WHERE id_import_history='.(int)$id_import_history);
        }
    }
    public function importData14($file_xml,$class_name,$foreign_key=array())
    {
		...
		{	
            if(isset($xml->$table) && $xml->$table)
            {
                ...
                {
                    $object_id=0;
                    if(!Db::getInstance()->getRow('SELECT * FROM '._DB_PREFIX_.'ets_pres2pres_'.pSQL($table).'_import WHERE id_old='.(int)$data_xml->$primary.' AND id_import_history="'.(int)$id_import_history.'"',false))
                    {
                        $imported= (int)Db::getInstance()->getValue('SELECT number_import FROM '._DB_PREFIX_.'ets_pres2pres_import_history WHERE id_import_history='.(int)$id_import_history,0);
                        ...
                        if($class_name=='Employee')
                        {
                            ...
                            {
                                Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'ets_pres2pres_employee_import values("","'.(int)$data_xml->$primary.'","'.(int)$id_employee.'","'.(int)$id_import_history.'")');
                                $imported++;
                                Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'ets_pres2pres_import_history SET number_import ="'.(int)$imported.'" WHERE id_import_history='.(int)$id_import_history);
                                continue;
                            }    
                        ...
                        if($foreign_key)
                        {
                            foreach($foreign_key as $key=>$value)
                            {
                                if(isset($object->$key)&&$object->$key)
                                {
                                    if($key=='id_parent')
                                    {
                                        $object->$key = (int)Db::getInstance()->getValue('SELECT id_new FROM '._DB_PREFIX_.'ets_pres2pres_'.pSQL($value['table_parent']).'_import WHERE id_old='.(int)$object->$key.' AND id_import_history="'.(int)$id_import_history.'"');
                                    }
                                    elseif($key_extra = (int)Db::getInstance()->getValue('SELECT id_new FROM '._DB_PREFIX_.'ets_pres2pres_'.pSQL($value['table_parent']).'_import WHERE id_old='.(int)$object->$key.' AND id_import_history="'.(int)$id_import_history.'"'))
                                    {
                                        $object->$key=$key_extra;
                                    }
                                    elseif(!Db::getInstance()->getValue('SELECT id_new FROM '._DB_PREFIX_.'ets_pres2pres_'.pSQL($value['table_parent']).'_import WHERE id_import_history="'.(int)$id_import_history.'"',0))
                                    {
                                        if($value['table_parent']=='category')
                                            $object->$key = $import_history['id_category_default'];
                                        elseif($value['table_parent']=='supplier')
                        ...
                        if($fileds_unquies)
                        {
                            ...
                            foreach($unquies as $unquie)
                            {
                                ...
                                foreach($unquie as $value)
                                {
                                    $sql .=' AND `'.pSQL($value).'` ="'.pSQL($object->$value,true).'"';
                                }
                            ...
                            foreach($languages as $language)
                            {
                                if(!in_array($language['id_lang'],$list_language_xml))
                                {
                                    foreach($fields_lang as $field)
                                    {
                                        ...
                                        {
                                            $temp = $object->$key;
                                            if($fieldsRequiredLang && in_array($key,$fieldsRequiredLang ) && (!isset($temp[$language['id_lang']]) || (isset($temp[$language['id_lang']])&& !$temp[$language['id_lang']])))
                                            {
                                                if(isset($language_xml_default) && $language_xml_default && isset($language_xml_default->$key) &&  (string)$language_xml_default->$key!='')
                                                {
                                            ...
                                            if(isset($fieldsValidateLang[$key])&& method_exists('Validate',$fieldsValidateLang[$key]) && ( !isset($temp[$language['id_lang']]) ||  $temp[$language['id_lang']]!='') ) 
                                            {
                                                $validate = $fieldsValidateLang[$key];
                        ...
                        if($class_name=='Customer' && $object->is_guest==0)
                        {
                            ...
                            {
                                $fields = Db::getInstance()->ExecuteS('DESCRIBE '._DB_PREFIX_.'customer');
                                $check_add=true;
                                foreach($fields as $field)
                                {
                                    if($field['Field']=='passwd_old_wp')
                        ...
                        if($class_name=='Employee')
                        {
                            ...
                            {
                                $fields = Db::getInstance()->ExecuteS('DESCRIBE '._DB_PREFIX_.'employee');
                                $check_add=true;
                                foreach($fields as $field)
                                {
                                    if($field['Field']=='passwd_old_wp')
                        ...
                        if($imported%100==0)
                        {                   
                            die('imported '.$class_name.'='.$imported);
                        }
                    }
                }
            }
...
//                        }
    ...
    public function copyImg14($id_entity, $id_image = null, $url, $entity = 'products')
    {
		...
		$url_source_file = self::file_get_contents($url_source_file);
